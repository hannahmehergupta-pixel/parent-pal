...
export default function ResultsPage({
  ...
  const getGoalDetails = (type: GoalType) => {
    switch (type) {
      case 'Education':
        return { label: '🎓 Higher Education', color: 'text-primary-700 bg-primary-50 border-primary-200 bg-primary-50/40' };
      case 'Medical Buffer':
        return { label: '🏥 Medical Buffer Shield', color: 'text-indigo-705 bg-indigo-50 border-indigo-200' };
      case 'Future Business':
        return { label: '💼 Business Setup Capital', color: 'text-amber-700 bg-amber-50 border-amber-200 bg-amber-50/40' };
    }
  };

  const goalMeta = getGoalDetails(goalType);

  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-8 md:py-12 text-stone-850">
      {/* Upper Navigation Row */}
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-stone-200/60 font-sans">
        <button
          id="results-back-btn"
          onClick={onStartOver}
      ...
      <div className="mb-10 text-center sm:text-left">
        <div className="p-8 sm:p-10 bg-[#fdfbf9] border border-stone-200/80 rounded-3xl shadow-sm relative overflow-hidden">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 mb-4">
            ...
            {inflationRate > 0 && (
              <span className="text-[10px] uppercase font-mono tracking-wider text-rose-750 bg-rose-50 border border-rose-150 px-3 py-1 rounded-full font-bold inline-flex items-center gap-1">
                ...
                Indexed {inflationRate}% Inflation Adjusted
              </span>
            )}
          </div>

          {/* Hero line requested precisely: "To have ₹[amount] by [year] ([N] years away), invest ₹[monthly] / month" */}
          <h2 id="results-hero-line" className="text-2xl sm:text-4xl font-extrabold font-display leading-tight text-stone-900 tracking-tight mb-5">
            To achieve your <span className="text-primary-700">{formatINR(plan.targetAmount)}</span> target (adjusted to <span className="text-primary-700 font-extrabold">{formatINR(plan.adjustedTargetAmount)}</span>) by <span className="text-primary-700">{plan.targetYear}</span> ({plan.timelineYears} {plan.timelineYears === 1 ? 'year' : 'years'} away), invest <span className="bg-gradient-to-r from-primary-600 to-indigo-700 bg-clip-text text-transparent">{formatINR(plan.monthly)}</span> / month
          </h2>

          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-6 text-xs text-stone-500 font-semibold">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-stone-200">
              <span className="text-stone-400 font-medium">Child's Age:</span>
              <span className="text-stone-850 font-bold">{childAge} yrs old</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-stone-200">
              <span className="text-stone-400 font-medium">Accumulation Horizon:</span>
              <span className="text-stone-850 font-bold">{plan.timelineYears} years</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-stone-200 animate-pulse">
              <span className="text-stone-400 font-medium">Original Target:</span>
              <span className="text-stone-600 line-through font-bold">{formatINR(plan.targetAmount)}</span>
            </div>
        ...
        {plan.recommendations.map((inst, index) => (
          <div key={inst.name} className="h-full">
            <Card
              ...
              className="flex flex-col h-full bg-white border border-stone-250/70 shadow-sm rounded-3xl hover:border-primary-300 hover:shadow-md transition-all duration-300 relative overflow-hidden"
            >
              ...
              <div className="p-6.5 sm:p-7 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  {/* Instrument title */}
                  <h4 className="text-base font-bold text-stone-850 tracking-tight leading-snug mb-2 pr-6">
                    {inst.name}
                  </h4>
      ...
      {/* Disclaimers */}
      <div id="statutory-disclaimers" className="p-5.5 bg-stone-50 border border-stone-200/80 rounded-2xl mb-12 flex items-start gap-3">
        ...
        <div className="text-[11px] leading-relaxed text-stone-500 font-sans">
          ...
          <p className="mb-1">
            * Market-linked returns are estimates, not guaranteed. This does not constitute specific registered financial advice.
          </p>
          Calculations are adjusted for your custom expected inflation rate of <strong>{inflationRate}% p.a.</strong> over <strong>{timelineYears} years</strong>. Sovereign limits, maximum monthly investment ceilings (e.g. PPF caps of ₹1.5L per annum), and real dynamic mutual fund indices fluctuate year-over-year. Please consult a qualified registered investment counselor before locking away actual funds.
        </div>
      </div>
